# Feature Specification: Flutter Library + Tester App Scaffold

**Feature Branch**: `001-flutter-library-scaffold`  
**Created**: 2026-06-05  
**Status**: Draft  
**Input**: User description: "create flutter lib: create empty flutter app that includes both widget library and a tester app for the library. use shadcn ui library in the project(s). placeholders are acceptable in this first iteration. just make sure the architecture is clean and maintainable from day one."

## Clarifications

### Session 2026-06-05

- Q: How should the monorepo be structured so the tester app depends on the library like an external consumer? → A: Dart pub workspaces (built-in `workspace:` resolution; library and tester app as separate sibling packages sharing one lockfile, no extra tooling).
...
- Q: What should the tester app's primary target platform be for this iteration? → A: macOS desktop (other platforms may be enabled later).
...
throwaway consumer (the tester app counts), importing only the public library file,
...
### User Story 2 - Run the tester app and see shadcn-styled output (Priority: P2)

A contributor checks out the repository, runs the tester app, and sees the library's
placeholder component rendered inside an application shell that is themed with the shadcn
UI design system. This proves the tester app exercises the library exactly as a real
consumer would and that the shadcn theming foundation is wired correctly.

**Why this priority**: A tester app that consumes the library as a real downstream app
...
**Independent Test**: Can be fully tested by launching the tester app on at least one
...
1. **Given** a freshly cloned repository, **When** a contributor runs the tester app, **Then** the app launches and displays the library's placeholder component inside a shadcn-themed shell.
2. **Given** the tester app, **When** its source is inspected, **Then** it references the library only through the public entry point and contains no duplicated copy of the library's internals.
3. **Given** the running tester app, **When** the contributor switches the shadcn theme (e.g., light/dark or a different color scheme), **Then** the placeholder component reflects the theme change, demonstrating the theming pipeline is live.
...
3. **Given** the placeholder component, **When** its widget test runs, **Then** it confirms the component builds and renders without depending on the tester app.
...
- How does the scaffold behave on a platform the tester app does not target? The supported target platforms must be stated, and unsupported platforms must fail fast with a clear message rather than render incorrectly.
...
- What happens when a new contributor clones the repo and runs setup? The path from clone to running tester app and passing tests must be documented and reproducible.
...
- **FR-002**: The project MUST contain a separate tester (sample) application that consumes the widget library solely through the library's public API.
...
- **FR-005**: Both the library and the tester app MUST integrate the `shadcn_ui` Flutter package as the shadcn UI design system, with the tester app demonstrating shadcn theming (via the ShadApp/ShadTheme pipeline) applied to the placeholder component.
...
- **Public API Surface**: The set of intentionally exported, documented symbols that consumers may use; the contract that the tester app and future consumers exercise.
- **Tester (Sample) Application**: A consumer of the library that renders the placeholder component within a shadcn-themed shell; exists to keep the public API honest and to provide a live validation surface.
...
- "shadcn UI library" refers to the community `shadcn_ui` Flutter package; it will be integrated as the theming/component foundation for the tester app and made available to the library.
- The repository is organized as a Dart pub workspace containing the library package and the tester app as separate sibling packages sharing one lockfile, so the tester app depends on the library the same way an external consumer would.
- The tester app's primary target platform for this iteration is macOS desktop (chosen for fast native iteration); other platforms may be enabled in a later iteration.
