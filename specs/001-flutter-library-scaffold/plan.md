# Implementation Plan: Flutter Library + Playground App Scaffold
...
(`jet_print`) and a separate playground app (`jet_print_playground`) that consumes it exactly as an
...
playground app renders a theme-aware placeholder component inside a `ShadApp`/`ShadTheme` shell
...
**Target Platform**: macOS desktop (playground app); library is platform-agnostic
...
|---|-----------|--------|------------------------|
| I | Library-First & Clean Public API | ✅ PASS | Library is a standalone workspace package with a single public entry point; internals under `src/`; playground app consumes only the public API (FR-001/002/003/011). |
...
| VI | Documentation & Developer Experience | ✅ PASS | Every public symbol gets dartdoc; runnable playground app + quickstart/README; `dart format` + strict `analysis_options` enforced (FR-009/010). |
...
├── pubspec.yaml                    # workspace: [packages/jet_print, apps/jet_print_playground]; sdk ^3.6.0
...
library and the single source of truth for the product; `apps/jet_print_playground` is a
sibling consumer package. Both declare `resolution: workspace`; the root `pubspec.yaml`
enumerates them under `workspace:` and produces one `pubspec.lock`, so the playground resolves
