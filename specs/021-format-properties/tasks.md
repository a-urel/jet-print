...
- [x] T052 (PENDING — requires a human at the GUI) Manual quickstart validation: walk all of `specs/021-format-properties/quickstart.md` in `apps/jet_print_playground` (style text → shape fill/outline/none/width-0 → barcode tint → preview/export/save-reload parity, undo at each step ≤3 interactions per SC-001/SC-006) — CLOSED 2026-06-20 via Epic 1; see acceptance-T052.md.
