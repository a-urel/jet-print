...
void main() {
  group('gradientBmp', () {
    test('has a well-formed BMP header and exact size', () {
      final Uint8List b = gradientBmp(
          width: 8, height: 6, topRgb: 0xFF0000, bottomRgb: 0x0000FF);
      // 'BM' magic.
      expect(b[0], 0x42);
      expect(b[1], 0x4D);
    ...
    testWidgets('decodes through the Flutter codec to the requested size',
      ...
      final Uint8List b = gradientBmp(
          width: 12, height: 9, topRgb: 0xE8B04B, bottomRgb: 0x7A3B12);
    ...
    test('base64 variant round-trips to the same bytes', () {
      final Uint8List raw = gradientBmp(
  ...
  group('foodBmp', () {
    test('every FoodIcon paints a glyph over the plain gradient', () {
      ...
      final Uint8List plain = gradientBmp(
          width: 64, height: 64, topRgb: 0xE8C07A, bottomRgb: 0xB6772E);
