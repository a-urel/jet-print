...
class FilledBand {
  ...
  /// The resolved element copies.
  final List<ReportElement> elements;

  /// The calculator's frozen variable values at this instance.
  ...
  @override
  int get hashCode => Object.hash(type, height, Object.hashAll(elements),
      Object.hashAll(<Object?>[
        for (final MapEntry<String, JetValue> e in variables.entries) ...<Object?>[
          e.key,
          e.value,
        ],
      ]));
...
class ReportFiller {
  ...
  FillResult fill(
    ...
    final Set<String> varGroupPageRefs = <String>{};
    ...
    // sharing the diagnostics + dedup set, and a shared page-scoped sink.
    ...
    // Page-scoped references recorded during variable/group evaluation (§5).
    for (final String name in varGroupPageRefs) {
      diagnostics.error(
        'Page-scoped variable \$V{$name} is not allowed in a variable or group '
        'expression',
      );
    }
...
Expected: PASS (10 tests); `No issues found!`.
