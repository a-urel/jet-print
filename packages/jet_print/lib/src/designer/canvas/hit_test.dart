...
///
/// Paint order is band order, then element order within a band; the element
/// drawn **last** is on top, so we scan in reverse and return the first hit.
/// [slop] (in points) enlarges each element's hit area beyond its visual bounds
/// so very thin or tiny elements (e.g. a zero-height line) remain grabbable
/// (FR-009 edge case).
String? hitTestElement(
  ...
  for (int b = template.bands.length - 1; b >= 0; b--) {
    final elements = template.bands[b].elements;
