...
JetReportDesignerController _controller({String? dataField}) =>
    JetReportDesignerController(
      definition: oneBandReport(
          elements: <ReportElement>[_element(dataField: dataField)]),
    );
...
void main() {
  group('setBarcodeSymbology', () {
    test('updates symbology, preserving data/bounds', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').symbology,
          BarcodeSymbology.ean13);
      expect(elementById<BarcodeElement>(c, 'b1').data, 'INITIAL'); // preserved
      expect(elementById<BarcodeElement>(c, 'b1').bounds, _bounds);
      c.dispose();
    });
  ...
  group('setBarcodeData', () {
    test('sets literal data and clears dataField', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').data, 'LITERAL');
      expect(elementById<BarcodeElement>(c, 'b1').dataField, isNull);
      c.dispose();
    });
  ...
  group('setBarcodeDataField', () {
    test('sets the bound field and is undoable', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').dataField, 'sku');
      expect(c.canUndo, isTrue);
      c.dispose();
    });
  ...
  group('setBarcodeValue (field-or-literal single input)', () {
    ...
    test('plain text is a literal and clears any bound field', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').data, '9501101530003');
      expect(elementById<BarcodeElement>(c, 'b1').dataField, isNull);
      c.dispose();
    });
    ...
    test('binding keeps the prior literal as a fallback', () {
      ...
      final JetReportDesignerController c = _controller();
      c.setBarcodeValue('b1', '[sku]');
      expect(elementById<BarcodeElement>(c, 'b1').data, 'INITIAL');
      expect(elementById<BarcodeElement>(c, 'b1').dataField, 'sku');
      c.dispose();
    });
    ...
    test('a value that is not a bare token stays literal (no expressions)', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').data, 'SKU-[sku]-X');
      expect(elementById<BarcodeElement>(c, 'b1').dataField, isNull);
      c.dispose();
    });
  ...
  group('setBarcode options', () {
    test('toggles showText to false', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').showText, isFalse);
      c.dispose();
    });
    ...
    test('toggles quietZone to false', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').quietZone, isFalse);
      c.dispose();
    });
    ...
    test('sets QR ecc level', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').eccLevel,
    ...
    test('omitted options are unchanged', () {
      ...
      expect(elementById<BarcodeElement>(c, 'b1').quietZone, isTrue);
      expect(elementById<BarcodeElement>(c, 'b1').eccLevel,
