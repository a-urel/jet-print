...
// half-authored report has, and the only place the two lookups disagree.
...
void main() {
  ...
  test('the two lookups differ ONLY where there is no band under the point',
      () {
    // The property the stale comments erased. Inside a band they agree; in the
    // gap `bandIdAt` declines and `bandIdNear` snaps.
    expect(_layout.bandIdAt(const JetOffset(100, 40)),
        _layout.bandIdNear(const JetOffset(100, 40)));
    expect(_layout.bandIdAt(_inTheGap), isNull);
